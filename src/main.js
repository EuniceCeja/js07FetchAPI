...
const mostrarProductos = (productos) => {
    //Mando a llamar las llaves de title, price, description, category, image desde la API se pueden manipular de manera individual. Así, evitamos el uso de un ciclo forEach
    productos.map((productos) => { //El método map toma el array (objeto en JS) y crea nuevos arreglos que 
...
fetch("https://jsonplaceholder.typicode.com/posts", {
    ...
    method: "POST",
    //Creo el body de mi nuevo objeto, siguiendo las mismas claves de la API existente (userId, title, body)
    body: JSON.stringify({
        userId: 1986,
...
console.log(users);
console.log(`${user[1].nombre} con posición ${user[1].posicion}, trabaja en ${user[1].empresa}`); //Output
